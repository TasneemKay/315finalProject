...
//  RestaurantChoice
//
//  Created by Tasneem K on 3/24/22.
//
/*additional features: I changed the settings on the launch screen so it shows
 long enough to see the logo well. I couldn't think of anything else to add honestly
 
 *Disclaimer: "This app is developed as an educational project. Certain materials are
included under the fair use exemption of the U.S Copyright Law and have been prepared according to the multimedia fair use guidelines and are restricted from further use." https://www.copyright.gov/fair-use/more-info.html
 */
...
class ViewController: UIViewController {
    ...
    @IBOutlet weak var RSDesc: UITextView!
    @IBOutlet weak var RSGenre: UILabel!
    @IBOutlet weak var lblFav: UILabel!
    @IBOutlet weak var Favlabel: UISwitch!
    @IBOutlet weak var lblGenre: UILabel!
    @IBOutlet weak var RSPic: UIImageView!
    
    /*var RestArray = ["Chipotle", "ChoongMan Chicken", "The Block", "MC Donalds", "Pizza Hut", "Noodles & Company", "Texas Roadhouse", "Tropical Smoothie", "Vocelli Pizza", "Freddy's", "Chuy's", "Panda Express", "Halal Guys", "District Chicken & Gyro", "Panera Bread", "Taco Bell", "Qdoba", "Subway", "Shake Shack", "Rawr Sushi"]
    ...
    @IBAction func valChange(_ sender: Any) {
        if Favlabel.isOn{
            UserDefaults.standard.set(RestName.text, forKey: "Favorites")
        }
        else{
    ...
    @IBAction func ordOnline(_ sender: Any) {
        let browserApp = UIApplication.shared
        let ordUrl = globalSelected.RestSite
        let url = URL(string: ordUrl)
        browserApp.open(url!)
    
    }
    
    fileprivate func setLabels() {
        //let randomRS = RSArray.randomElement()
        let randomRS = globalSelected
        RSPic.image = UIImage(named: randomRS.RestPic)
        RestName.text = randomRS.ResName
        RSDesc.text = randomRS.RestDesc
        lblGenre.text = randomRS.RestGenre
        let setFav = UserDefaults.standard.string(forKey: "Favorites")
        Favlabel.isOn = (randomRS.ResName == setFav)
    }
...
//vocelli, noodles and co, the block
//
